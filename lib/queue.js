...
class Queue extends Emitter {
  ...
  checkHealth(cb) {
    const promise = this._commandable()
      .then((client) =>
        helpers.callAsync((done) =>
          (client.batch ? client.batch() : client.multi())
      ...
      .then((results) => {
        if (Array.isArray(results[0]) && results[0][0] === null) {
  ...
  _finishJob(err, data, job) {
    ...
    const multi = (
      this.client.batch ? this.client.batch() : this.client.multi()
    )
  ...
  saveAll(jobs) {
    return this._commandable().then((client) => {
      const batch = client.batch ? client.batch() : client.multi(),
        errors = new Map();
